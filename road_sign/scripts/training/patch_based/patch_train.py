...
def get_data_pairs(data_dir):
    """
    Recursively finds image-label pairs in the hierarchical patch directory structure.
    ...
    # Iterate through image subdirectories (named after original images)
    for subdir_name in os.listdir(img_dir):
...
def split_by_original_image(data_dir, train_ratio=0.9):
    """
    Splits the dataset ensuring patches from the same original image stay together.
    ...
    # Split the subdirectories (original images)
    import random
...
def main():
    ...
    # --- Configuration ---
    # We use the newly proposed adaptive patch directory
    DATA_DIR = os.path.join(road_sign_root, 'data_patch_adaptive')
    ARTIFACT_DIR = os.path.join(road_sign_root, 'artifacts', 'patch_based')
    IMG_SIZE = (512, 512)
    BATCH_SIZE = 256
    EPOCHS = 50
    NUM_CLASSES = 8
    ...
    # 1. Prepare Data Pairs & Split
    # Since patches from the same image are grouped in subdirectories, 
    # we split by original image to prevent data leakage.
    ...
    train_ds = YoloFormatDataset(train_pairs, IMG_SIZE, train_transforms)
    val_ds = YoloFormatDataset(val_pairs, IMG_SIZE, val_transforms)
