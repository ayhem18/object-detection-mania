...
from home_made_od.yolo_v2.modules.yolov2_diagnosis import analyze_gt_matches, DiagnosisDataset
...
def main():
    ...
    # 6. Prepare Diagnosis Dataset (for easy orig_size access)
    val_img_paths = [p[0] for p in val_pairs]
    diag_ds = DiagnosisDataset(val_img_paths, IMG_SIZE, preprocess)
    
    # 7. Run GT Analysis loop
    all_diagnostics = []
    
    # We use batch size 1 to easily handle different numbers of GT boxes per image
    dataloader = DataLoader(diag_ds, batch_size=4, shuffle=False, num_workers=2)
    ...
    for batch_tensors, batch_idxs in tqdm(dataloader, desc="GT Matching Analysis"):
        ...
        for idx_tensor in batch_idxs:
            ...
            # Label format: cls, cx, cy, w, h (normalized)
            gt = []
            if os.path.exists(label_path):
                with open(label_path, 'r') as f:
                    for line in f:
                        ...
                        # Convention: orig_w is height (y), orig_h is width (x)
                        # So cx * orig_h, cy * orig_w
                        x1 = (ncx - nw/2) * orig_h
                        y1 = (ncy - nh/2) * orig_w
                        x2 = (ncx + nw/2) * orig_h
                        y2 = (ncy + nh/2) * orig_w
        ...
        batch_diags = analyze_gt_matches(
            model=model,
