"""
Train RetinaNet on a road-sign **resized** dataset.

Prerequisites::

    uv run python road_sign/scripts/data_scripts/resize_dataset.py
    uv run python road_sign/scripts/data_scripts/create_split.py \\
        --dataset-version resized --model-name retinanet

Run from the monorepo root::

    uv run python road_sign/scripts/retinanet/training/train_resized.py
"""
...
from home_made_od.general.path_utils import DATASET_VERSION_RESIZED

# Monorepo root on sys.path (same pattern as other road_sign training scripts).
...
DEFAULT_CONFIG: dict = {
    "experiment_name": "retinanet_resized",
    "dataset_version": DATASET_VERSION_RESIZED,
    "dataset_hash": None,
    "split_hash": None,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RetinaNet training on resized road-sign data.")
...
def main() -> None:
    ...
    artifact_dir = run_retinanet_training(DATASET_VERSION_RESIZED, config)
